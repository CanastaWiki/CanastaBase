...
for db_name in "${ids[@]}"; do
    # Create the cache, images, public_assets, and sitemap directories if they don't exist
    mkdir -p $MW_VOLUME/cache/$db_name
    mkdir -p $MW_VOLUME/images/$db_name
    mkdir -p $MW_VOLUME/public_assets/$db_name
    mkdir -p $MW_VOLUME/public_assets/$db_name/sitemap
    ...
    chown $WWW_USER:$WWW_GROUP $MW_VOLUME/public_assets/$db_name/sitemap
done
