    ...
    for wiki in $wikis; do
        ...
        echo "$wiki_id transcoder started."
        {
            while true; do
                ...
                date >> "$logfileNow"
                php "$RJ" --type webVideoTranscodePrioritized --maxjobs=10 --server="https://$wiki_url" --wiki="$wiki_id" >> "$logfileNow" 2>&1
                sleep 1
                php "$RJ" --type webVideoTranscode --maxjobs=1 --server="https://$wiki_url" --wiki="$wiki_id" >> "$logfileNow" 2>&1
